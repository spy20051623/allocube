...
本次日历编辑序列属于网页功能。官方 API 的 `UPDATE` 仍是单条修改：进行中占用只允许修改说明，不能调整时间；未开始占用可在原资源组和范围内改期。官方 API 不接受网页使用的 `replaceReservations` 或 `expectedStateToken`，不能直接调用网页的批量替换接口；如需先取消再创建两个独立 API 操作，它们不构成一个原子事务。
