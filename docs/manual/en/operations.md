...
Clients coalesce events and post-operation refreshes within 200 milliseconds, keeping one request in flight and at most one trailing refresh per query. Hidden pages pause ordinary reads and disconnected fallback polling, then synchronize when visible; revoked access is handled immediately. Legacy or unknown events fall back to a coalesced sync of open modules, and old clients still recognize `revision` events. System settings synchronize automatically when there is no unsaved input. Drafts are kept when remote settings change; a version conflict on submission asks whether to overwrite the corresponding settings. Cancelling keeps the draft. There is no dedicated reload button; users can refresh the page manually.
...
Settings and session reads have a 15-second deadline, including response-body reads. Failed reads retry after 2, 5, 15, then 30 seconds, remaining at 30-second intervals; hidden pages pause actual queries. Settings forms remain unavailable until the initial snapshot is complete, and editing pauses during saves and overwrite confirmation. Reads started before a save cannot overwrite its result. Older versions and incomplete responses are rejected and reread. Network failures do not imply logout; failed authority rechecks keep restricted content hidden.

Settings writes are never retried automatically. If a timeout, disconnect, or server error leaves an operation unconfirmed, input is retained and further submission is blocked until the user manually refreshes the page and checks the result. An explicit version conflict returns 409; only user confirmation submits `overwrite: true`. This option applies only to internal system settings endpoints, retaining system-admin authorization, CSRF and field validation, transactional writes, and audit records. After building, run `npm run test:settings-network` for browser fault injection. It requires Playwright and a Chromium browser; `PLAYWRIGHT_MODULE` can specify an existing Playwright package path, and `PLAYWRIGHT_CHANNEL` a browser channel such as `msedge`. The isolated-database runner covers slow responses, partial initial failures, late reads, lost save responses, cancelled and confirmed overwrites, edits during confirmation, drafts across sections, offline and hidden states, timeout recovery, and failed authority rechecks.

Calendar draft previews use the coalesced refresh queue scoped to relevant machines and times, with a 15-second read deadline, retries after 5 seconds, and no reads while hidden. Responses apply only to their original draft snapshot and do not replace a time field being edited. Internal calendar preview and submission use `autoAdjust: true` to trim or split available slots and notify the user. Final adjustment, creation, and replacement of originals share one immediate transaction. No remaining slots, more than 100 slots, changed originals, or write failures do not release originals. Official `/api/open/v1` confirmation-token and conflict rules remain unchanged and do not enable automatic adjustment. After building, run `npm run test:calendar-adjustment` for isolated browser acceptance using the Playwright configuration above.

Documentation scripts and styles load only when visiting `/docs`; ordinary pages do not preload the complete manual. If loading exceeds 15 seconds or an asset request fails, **Retry** reloads the current URL, preserving the chapter and anchor. After building, run `npm run test:docs-loading` to check on-demand loading, Chinese and English, narrow screens, history navigation, and retries after script, stylesheet, or timeout failures. Use the Playwright configuration above.

HTML entry points and unversioned static files use `no-cache` with ETag validation; unchanged responses return `304` and reuse the browser cache. Content-hashed files under `/assets/` use a one-year `immutable` cache and new filenames on updates, so only changed files need downloading. Business APIs retain `no-store`; reservation and permission data are not cached. Missing assets return an uncacheable `404`, never HTML in place of a script. Proxies should preserve these headers rather than apply long-lived caching to every path. Publishing does not automatically refresh open tabs; opening or refreshing the page checks for a new version. After building, run `npm run test:browser-cache` to verify actual browser cache hits using the Playwright configuration above.
