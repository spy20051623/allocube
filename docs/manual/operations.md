...
客户端将实时事件和操作后的刷新合并到 200 毫秒窗口，同一查询仅保留一个在途请求和一次后续刷新。隐藏页面暂停普通查询及断线兜底，恢复前台后统一同步；失权处理不等待页面可见。旧格式或未知事件回退为当前已打开模块的合并同步，旧客户端仍能读取 `revision` 事件。系统设置有未保存输入时，远端修改会提示重新加载核对，不覆盖输入。
