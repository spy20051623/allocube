...
it("upgrades version 20 in place, preserves audit payloads and restores a backup",async()=>{
  const legacy=new Database(process.env.DATABASE_PATH!);legacy.exec(SCHEMA_V21_SQL.replace(AUDIT_INDEX_SQL,""));
  ...
  expect(database.db.prepare("SELECT MAX(version) AS n FROM schema_migrations").get()).toEqual({n:FINAL_SCHEMA_VERSION});
